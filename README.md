...
local target inventory
  ...
  -> bounded retry attempts with immutable attempt evidence
...
Retry execution is explicit and failed-occurrence-only. It preserves the original failed execution as immutable attempt evidence, uses deterministic attempt IDs, and enforces a hard maximum of three attempts. The operator page is read-only; retry mutation remains API-only.
...
WATCH is read-only first. Current controls include explicit target registration, bounded timeouts and redirects, public-address validation, direct connections to validated IP addresses, normal TLS verification, disabled-target guards, startup-configured workspace isolation, local-only action transitions, bounded explicit retry, and read-only operator pages.

WATCH does not bypass authentication, submit external forms, crawl sites, store credentials, automatically retry in a background loop, install Task Scheduler jobs, execute batches, or modify external systems.
...
The next major bounded slice is M3.3: a rollback-safe Windows Task Scheduler adapter for one due-runner task, with dry-run, install, verify, uninstall, and scheduled proof packaging. The explicitly approved live-report example remains a separate evidence task.